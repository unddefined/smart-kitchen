...
async function fillDishesFinal() {
  console.log('🌱 开始最终菜品数据填充...');
  ...
  try {
    ...
    const menuItems = [
      ...
      { name: '椒盐排骨', category: '中菜', station: '热菜', shortcutCode: 'JYGP', countable: true },
      ...
      { name: '托炉饼', category: '中菜', station: '热菜', shortcutCode: 'TLB', countable: true },
      ...
      { name: '铁板豆腐', category: '中菜', station: '热菜', shortcutCode: 'TBDF', countable: true },
      { name: '沙拉牛排', category: '中菜', station: '热菜', shortcutCode: 'SLNP', countable: true },
      
      // 点心类
      { name: '小笼馒头', category: '点心', station: '点心', shortcutCode: 'XLMT', countable: true },
      { name: '手工米糕', category: '点心', station: '点心', shortcutCode: 'SGMG', countable: true },
      
      // 蒸菜类
      { name: '红蒸湘鱼', category: '蒸菜', station: '蒸菜', shortcutCode: 'HZXY', countable: false },
      { name: '蒜蓉小鲍鱼', category: '蒸菜', station: '蒸菜', shortcutCode: 'TRXBY', countable: true },
      ...
      { name: '乌米饭', category: '蒸菜', station: '蒸菜', shortcutCode: 'WFM', countable: true },
    ...
    for (const item of menuItems) {
      ...
      try {
        ...
        const countableText = item.countable ? ' (计数)' : '';
        console.log(`✅ 添加菜品: ${item.name}${countableText} (${item.category}, ${item.station})`);
        addedCount++;
      } catch (error) {
    ...
    // 显示需要计数的菜品
    console.log('\n🔢 需要计数的菜品:');
    const countableItems = await prisma.dish.findMany({
    ...
    for (const dish of countableItems) {
