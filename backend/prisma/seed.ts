...
async function main() {
  ...
  const dishes = [
    // 凉菜类
    { name: '三文鱼拼鹅肝', stationId: stationMap['凉菜'], categoryId: categoryMap['凉菜'], shortcutCode: 'SWSPGH', recipeId: null },
    { name: '美味八碟', stationId: stationMap['凉菜'], categoryId: categoryMap['凉菜'], shortcutCode: 'MWBD', recipeId: null },
    
    // 前菜类
    { name: '藜麦元宝虾', stationId: stationMap['热菜'], categoryId: categoryMap['前菜'], shortcutCode: 'LMYYX', recipeId: null },
    { name: '盐水河虾', stationId: stationMap['热菜'], categoryId: categoryMap['前菜'], shortcutCode: 'YSHX', recipeId: null },
    ...
    await prisma.orderItem.create({
      data: {
        ...
        countable: itemData.countable ?? (
          createdDishes.find(d => d.id === itemData.dishId)?.name.includes('托炉饼') || 
