...
export class OrdersService {
   ...
   async create(createOrderDto: any) {
      ...
      if (createOrderDto.mealTime) {
         try {
            ...
            throw new Error(`用餐时间格式错误：${typeof createOrderDto.mealTime === "string" ? createOrderDto.mealTime : "invalid date"}`, error);
         }
      }

      // 处理 mealType（餐型）
      if (createOrderDto.mealType) {
         mealTypeValue = createOrderDto.mealType as "lunch" | "dinner" | "breakfast" | "other";
   ...
   async findAll(queryParams: FindAllQueryParams) {
      ...
      const updatedOrders = await Promise.all(
         orders.map(async (order) => {
            const updatedOrder = await this.kitchenService.checkAndUpdateOrderStatus(order);
            return {
               ...updatedOrder,
      ...
      if (queryParams.mealType) {
         filteredOrders = filteredOrders.filter((order) => order.mealType === queryParams.mealType);
      }
   ...
   async update(id: number, updateOrderDto: any) {
      ...
      if (updateOrderDto.status && updateOrderDto.status !== order.status) {
         if (!this.kitchenService.validateStatusTransition(order.status, updateOrderDto.status)) {
            throw new Error(`无法从 ${order.status} 状态更新为 ${updateOrderDto.status}`);
         }
      }
      ...
      // 只有当状态变更时，才更新订单菜品
      if (updateOrderDto.status && updateOrderDto.status !== order.status) await this.kitchenService.updateOrderItemsByStatus(updatedOrder);
   ...
   async cancelOrder(id: number) {
      ...
      // 检查订单状态，不能取消已完成的订单
      if (order.status === "done" || order.status === "cancelled") throw new Error("无法取消已完成或已取消的订单");
   ...
   async addOrderItem(orderId: number, createOrderItemDto: any) {
      return await this.orderItemsService.addOrderItem(orderId, createOrderItemDto);
   }
   ...
   async updateOrderItem(orderId: number, itemId: number, updateData: any) {
      return await this.orderItemsService.updateOrderItem(orderId, itemId, updateData);
   }
}
