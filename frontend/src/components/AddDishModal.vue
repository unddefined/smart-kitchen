   ...
   <div v-if="showAddNewDish" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl w-full max-w-md">
         ...
         <div class="p-4 border-t bg-gray-50 flex space-x-3">
            <button @click="closeAddNewDish" class="flex-1 py-3 bg-gray-200 text-black rounded-lg font-medium hover:bg-gray-300 transition-colors">
               取消
            </button>
            <button
...
const canSubmit = computed(() => {
   return selectedOrder.value && (orderedSelectedDishes.value.length > 0 || availableSelectedDishes.value.length > 0);
});
