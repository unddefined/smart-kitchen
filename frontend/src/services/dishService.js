...
export class DishService {
   ...
   static async getDishesByStation(stationId) {
      try {
         const dishes = await api.dishes.list();
         return dishes.filter((dish) => dish.stationId === stationId).sort((a, b) => a.name.localeCompare(b.name, "zh-CN"));
      } catch (error) {
   ...
   static async getDishesByCategory(categoryId) {
      try {
         const dishes = await api.dishes.list();
         return dishes.filter((dish) => dish.categoryId === categoryId).sort((a, b) => a.name.localeCompare(b.name, "zh-CN"));
      } catch (error) {
      ...
      try {
         ...
         try {
            ...
            return allDishes.filter(
               (dish) => dish.name.toLowerCase().includes(searchTerm) || (dish.shortcutCode && dish.shortcutCode.toLowerCase().includes(searchTerm)),
            );
         }
      } catch (error) {
