...
export class ServingService {
  ...
  static async urgeDish(itemId, reason = '客户催菜') {
    try {
      const result = await api.orderItems.updatePriority(itemId, PRIORITY_LEVELS.URGENT, reason)
      return {
        success: true,
        message: '催菜成功',
        data: result
  ...
  static async delayDish(itemId, reason = '暂缓制作') {
    try {
      const result = await api.orderItems.updatePriority(itemId, PRIORITY_LEVELS.WAIT, reason)
      return {
        success: true,
        message: '暂缓成功',
        data: result
