...
const dishManager = useDishManager({
  onStatusChange: (dish, newStatus, newPriority) => {
    console.log("状态变更回调:", dish.name, newStatus, newPriority);
  },
  onPriorityAdjust: (dish, quantity, priority) => {
    console.log("优先级调整回调:", dish.name, quantity, priority);
  },
});
...
const onDishDoubleClick = (dish) => {
  handleDishDoubleClick(dish, emit);
};
...
const onServedDishClick = (dish) => {
  handleServedDishClick(dish, emit);
};
