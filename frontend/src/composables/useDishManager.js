      ...
      try {
         if (dish.status === "pending") {
            ...
            if (needPrep === false) {
               ...
               // 批量操作
               if (itemIds.length > 1) {
                  const results = await Promise.all(itemIds.map((id) => api.serving.completePrep(id)));
               ...
               // 批量操作
               if (itemIds.length > 1) {
                  const results = await Promise.all(itemIds.map((id) => ServingService.startPreparation(id)));
                  const successCount = results.filter((r) => r?.success).length;
                  result = {
                     success: true,
                     count: successCount,
