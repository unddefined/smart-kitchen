...
export const useServingStore = defineStore("serving", {
  ...
  getters: {
    ...
    statistics: (state) => {
      ...
      return {
        ...
        pendingItems,
        completionRate:
          totalItems > 0 ? Math.round((servedItems / totalItems) * 100) : 0,
      };
    },
  },
  ...
  actions: {
    ...
    async completePreparation(itemId) {
      try {
        const response = await fetch(
          `/api/serving/items/${itemId}/complete-prep`,
          {
            method: "POST",
          },
        );
    ...
    async autoAdjustOrderPriorities(orderId) {
      try {
        const response = await fetch(
          `/api/serving/orders/${orderId}/auto-adjust`,
          {
            method: "POST",
          },
        );
    ...
    selectOrder(orderId) {
      this.currentOrder =
        this.orders.find((order) => order.orderId === orderId) || null;
    },
