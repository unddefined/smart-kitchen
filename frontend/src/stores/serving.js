...
export const useServingStore = defineStore("serving", {
   ...
   getters: {
      // 获取需要催菜的订单
      urgentOrders: (state) => {
         return state.orders.filter((order) => order.items?.some((item) => item.currentPriority < item.shouldPriority || item.isAddedLater));
      },
   ...
   actions: {
      ...
      async updateItemPriority(itemId, priority, reason) {
         try {
            ...
            const data = await response.json();
            console.log(data.message || "优先级更新成功");
      ...
      async completePreparation(itemId) {
         try {
            ...
            const data = await response.json();
            console.log(data.message || "标记完成成功");
      ...
      async serveDish(itemId) {
         try {
            ...
            console.error("上菜失败: " + error.message);
            throw error;
         }
      },
      ...
      async autoAdjustOrderPriorities(orderId) {
         try {
            ...
            const data = await response.json();
            console.log(data.message || "自动调整成功");
            ...
            console.error("自动调整失败: " + error.message);
            throw error;
         }
      },
